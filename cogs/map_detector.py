...
log = logging.getLogger("MapDetector")

class MapDetector(commands.Cog):
    ...
    async def notify_users(self):
        ...
        users_to_notify : Dict[int, List[int]] = {} # server_id -> list of user ids
            ...
            for user_id, filters in self.user_filters.items():
                for filter in filters:
                    if filter.apply(server) and server_id not in self.sent_notifications.setdefault(int(user_id), set()):
                        if server_id not in servers_for_notifications:
                            servers_for_notifications[server_id] = server
                        
                        if server_id not in users_to_notify:
                            users_to_notify[server_id] = [int(user_id)]
                        else:
                            users_to_notify[server_id].append(int(user_id))
    ...
    async def send_notifications(self, servers: Dict[int, dict], users_to_notify: Dict[int, List[int]]):
        ...
        for server_id, server in servers.items():
            ...
            embed = discord.Embed(
                ...
                description=f"A server has been found matching your criterias.\n Mentions: {', '.join(mentions)}",
            ...
            self.bot.dispatch("server_match", embed, file)
